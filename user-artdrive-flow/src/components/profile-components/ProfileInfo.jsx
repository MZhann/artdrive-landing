...
const ProfileInfo = ({ userData }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const newSocialLinks = Object.fromEntries(formData.socialLinks);

            if (JSON.stringify(currentSocialLinks) !== JSON.stringify(newSocialLinks)) {
                profileFormData.append("social_links", JSON.stringify(newSocialLinks));
            }


            setIsLoading(true);
            await updateProfile(profileFormData);
            // console.log('changed params: ', profileFormData.getAll)
