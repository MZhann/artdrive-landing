...
// import TournamentEnded from "./TournamentEnded";
// import LikesAndChanceIndicator from "./LikesAndChanceIndicator";
// import { fetchArtworks, fetchCurrentRound, fetchUserImage, fetchTournamentDetails } from "@/pages/api/tournaments";
...
//         try {
//             const roundNumber = await fetchCurrentRound(tournamentId);
//             if (roundNumber.current_round === null) {
...
//     const determineStage = (currentRound, totalRounds) => {
//         if (currentRound === 1) {
//             return "group stage";
...
//     const startStage = async (roundNumber) => {
//         const stage = determineStage(roundNumber, totalRounds);
//         setTournamentState(stage);
//         await fetchArtworksToVote();
//         setShowRoundModal(true);
//         setTimeout(() => {
//             setShowRoundModal(false);
...
import TournamentEnded from "./TournamentEnded";
import LikesAndChanceIndicator from "./LikesAndChanceIndicator";
import { fetchArtworks, fetchCurrentRound, fetchUserImage, getTournamentInfo } from "@/pages/api/tournaments";
import { useRouter } from 'next/router';
...
const TournamentPage = ({
    ...
    useEffect(() => {
        const initialize = async () => {
            try {
                // Fetch tournament details and check status
                const tournamentInfo = await getTournamentInfo(tournamentId);
                if (tournamentInfo.status === 'past') {
                    router.push('/tournament-over');
                    return; // Stop further processing if the tournament is past
                }
